...
import { Scan, User, Plus, RefreshCw, X } from 'lucide-react';
...
const ScanMember = () => {
  ...
  const handleScanSuccess = (identifier: string) => {
  ...
  const handleAwardPoints = () => {
    ...
    adminAddCredits(scannedUser.id, parseInt(amount), reason);
    showSuccess(`${amount} points awarded to ${scannedUser.name}`);
    handleReset();
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-lg mx-auto pb-20">
      <div className="text-center">
        <div className="w-16 h-16 bg-brand-charcoal rounded-2xl shadow-neumorphic-out mx-auto mb-4 flex items-center justify-center">
          <Scan className="w-8 h-8 text-brand-silver" />
        </div>
        <h1 className="text-xl font-semibold">Member Scanner</h1>
        <p className="text-xs text-brand-silver font-light mt-1">
          {isScanning ? 'Position the member QR code within the frame.' : 'Confirm member details and award points.'}
        </p>
      </div>
          ...
          <div className="bg-brand-charcoal p-6 rounded-3xl shadow-neumorphic-out border border-brand-white/5 relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="w-14 h-14 rounded-full bg-brand-charcoal shadow-neumorphic-in p-1 border border-brand-white/5 overflow-hidden">
                <img src={scannedUser.avatar} alt="" className="w-full h-full object-cover rounded-full" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-brand-white">{scannedUser.name}</h3>
                <p className="text-[10px] text-brand-silver uppercase tracking-widest">{scannedUser.tier} Member</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="p-3 bg-brand-charcoal rounded-xl shadow-neumorphic-in">
                ...
                <p className="text-sm font-bold">{scannedUser.credits.toLocaleString()} pts</p>
              </div>
              <div className="p-3 bg-brand-charcoal rounded-xl shadow-neumorphic-in">
      ...
      <div className="pt-8 text-center">
        <p className="text-[10px] text-brand-silver/20 uppercase tracking-[0.3em]">Mobile Scanning Terminal</p>
      </div>
    </div>
  );
};
